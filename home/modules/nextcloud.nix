...
let
  ...
  reset = pkgs.writeShellScript "nextcloud-reset-mountpoint" ''
    ...
    # read-only again so a down mount cannot quietly collect files on the SSD --
    # but never on a non-empty dir: chmod 0555 over the old 38 GB sync folder
    # would only make it harder to delete.
    if [ -z "$(${pkgs.coreutils}/bin/ls -A ${mountPoint} 2>/dev/null)" ]; then
      ${pkgs.coreutils}/bin/chmod 0555 ${mountPoint}
    fi
  '';
in
{
  ...
  systemd.user.services = {
    ...
    "rclone-mount:@nextcloud" = {
      ...
      Service = {
        ...
        ExecStartPre = lib.mkForce [ "-${reset}" ];
  ...
  # dir underneath the mount is left read-only. rclone still mounts over it.
